    ...
    <SafeAreaView style={styles.safeArea}>
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.heroCard}>
          <View style={styles.cardHeader}><Text style={styles.darkHeaderText}>今日鼓励</Text><Text style={styles.darkBadge}>Mindset</Text></View>
          <Text style={styles.quote}>{moodQuotes[quoteIndex]}</Text>
          <Pressable onPress={refreshQuote} style={styles.darkSecondaryButton}><Text style={styles.darkSecondaryButtonText}>换一句</Text></Pressable>
        </View>

        <View style={styles.card}>
          <View style={styles.cardHeader}><Text style={styles.cardHeaderText}>今日状态</Text><Text style={styles.badge}>{today}</Text></View>
          <View style={styles.moodGrid}>
            {moodOptions.map((item) => (
              <Pressable key={item} onPress={() => setMood(item)} style={[styles.moodOption, mood === item && styles.moodOptionActive]}>
        ...
        <View style={styles.card}>
          <View style={styles.cardHeader}><Text style={styles.cardHeaderText}>日记</Text><Text style={styles.badge}>Local</Text></View>
          ...
          <TextInput
            ...
            value={note}
          />
          <Text style={styles.hint}>{mood || note ? "当前心情记录已保存在本页面。" : "心情和日记会保存在当前页面状态。"}</Text>
        </View>
      </ScrollView>
    </SafeAreaView>
  );
...
const styles = StyleSheet.create({
  ...
  darkSecondaryButton: { alignItems: "center", alignSelf: "flex-start", backgroundColor: "#ffffff", borderRadius: 18, justifyContent: "center", minHeight: 46, paddingHorizontal: 16, paddingVertical: 12 },
  darkSecondaryButtonText: { color: "#0a0a0a", fontWeight: "900" },
  moodGrid: { flexDirection: "row", flexWrap: "wrap", gap: 12 },
  moodOption: { alignItems: "center", backgroundColor: "#ffffff", borderRadius: 22, minHeight: 52, justifyContent: "center", minWidth: "47%", padding: 14 },
  ...
  textarea: { backgroundColor: "#f7f7f9", borderColor: "rgba(255,255,255,0.82)", borderRadius: 18, borderWidth: 1, color: "#0a0a0a", fontSize: 16, minHeight: 190, padding: 14 },
