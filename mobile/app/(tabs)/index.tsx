...
export default function TodayScreen() {
  ...
  if (isError || !data)
    return (
      <CenteredMessage text="今日数据加载失败" actionLabel="重试" onPress={() => void refetch()} />
    );
      ...
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.checkinCard}>
          <View>
            ...
            <Text style={styles.checkinText}>
              {hasLogged ? "已经完成一次记录，继续保持节奏。" : "先记一餐，完成今日饮食打卡。"}
            </Text>
          </View>
          <Link href="/record" asChild>
            <Pressable style={styles.primaryButton}>
              <Text style={styles.primaryButtonText}>去记一餐</Text>
            </Pressable>
          </Link>
        </View>
        ...
        <View style={styles.card}>
          ...
          <Text style={styles.gaugeHint}>
            {data.remaining_calories >= 0 ? "剩余热量 kcal" : "已超出目标 kcal"}
          </Text>
          <View style={styles.compactLine}>
            <Text style={styles.compactText}>
              已吃 <Text style={styles.compactStrong}>{Math.round(data.food_consumed)}</Text>
            </Text>
            <Text style={styles.compactText}>
              目标 <Text style={styles.compactStrong}>{Math.round(data.target_intake)}</Text>
            </Text>
          </View>
