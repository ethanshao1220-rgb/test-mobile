      ...
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={styles.flex}>
        <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
          ...
          <View style={styles.heroCard}>
            <View style={styles.cardHeader}><Text style={styles.darkHeaderText}>本周 vs 上周</Text><Text style={styles.darkBadge}>本周</Text></View>
          ...
          <View style={styles.card}>
            <View style={styles.cardHeader}><Text style={styles.cardHeaderText}>目标设置</Text><Text style={styles.badge}>Goal</Text></View>
            <View style={styles.segmented}>
              {periods.map((item) => (
                <Pressable key={item} onPress={() => setPeriod(item)} style={[styles.segment, period === item && styles.segmentActive]}>
                  <Text style={[styles.segmentText, period === item && styles.segmentTextActive]}>{item === "day" ? "每天" : item === "week" ? "每周" : "每月"}</Text>
                </Pressable>
              ))}
            </View>
            ...
            <View style={styles.buttonRow}>
              <Pressable style={styles.secondaryButton}><Text style={styles.secondaryButtonText}>重新计算</Text></Pressable>
              <Pressable disabled={mutation.isPending} onPress={savePlan} style={styles.primaryButton}>
                <Text style={styles.primaryButtonText}>{mutation.isPending ? "保存中" : "保存目标"}</Text>
              </Pressable>
            </View>
          </View>

          <View style={styles.card}>
            <View style={styles.cardHeader}><Text style={styles.cardHeaderText}>每日表现</Text><Text style={styles.badge}>7 days</Text></View>
            <DailyItem date="今日" status="目标范围内" rate="0%" />
            <DailyItem date="昨日" status="暂无数据" rate="0%" />
          </View>

          <View style={styles.card}>
            <View style={styles.cardHeader}><Text style={styles.cardHeaderText}>下周建议</Text><Text style={styles.badge}>Next</Text></View>
...
function DailyItem({ date, status, rate }: { date: string; status: string; rate: string }) {
  return (
    <View style={styles.dailyItem}>
      ...
      <Text style={styles.dailyStatus}>{rate} · {status}</Text>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  compareItem: { backgroundColor: "rgba(255,255,255,0.08)", borderRadius: 22, minWidth: "47%", padding: 16 },
  ...
  segment: { alignItems: "center", borderRadius: 999, flex: 1, minHeight: 44, justifyContent: "center" },
  ...
  primaryButton: { alignItems: "center", backgroundColor: "#050505", borderRadius: 18, flex: 1, justifyContent: "center", minHeight: 48, padding: 12 },
  primaryButtonText: { color: "#fff", fontWeight: "900" },
  secondaryButton: { alignItems: "center", backgroundColor: "#ffffff", borderRadius: 18, flex: 1, justifyContent: "center", minHeight: 48, padding: 12 },
  secondaryButtonText: { color: "#0a0a0a", fontWeight: "900" },
  dailyItem: { alignItems: "center", backgroundColor: "#ffffff", borderRadius: 22, flexDirection: "row", justifyContent: "space-between", padding: 15 },
